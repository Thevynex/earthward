...
/** Pure placement plan with component-level foundations and flat inferred building shells. */
public final class ChunkPlacementPlanner {
    ...
    public Plan plan(int chunkX, int chunkZ, int[] terrainSurfaceY) {
        ...
        for (int dx=-1; dx<=1; dx++) for (int dz=-1; dz<=1; dz++) {
            int x=chunkX+dx, z=chunkZ+dz; masks.put(key(x,z), rasterizer.rasterize(x,z));
        }
        var center=masks.get(key(chunkX,chunkZ));
        int[] buildingBase=buildingBases(center, terrainSurfaceY);
        ...
        for (int localZ=0; localZ<16; localZ++) for (int localX=0; localX<16; localX++) {
            int index=localZ*16+localX, worldX=originX+localX, worldZ=originZ+localZ;
            int surface=terrainSurfaceY[index];
            ...
            if (kind==ChunkFeatureRasterizer.Surface.ROAD) {
                ...
                int base=buildingBase[index];
                for (int y=surface; y<=base; y++) placements.add(new Placement(worldX,y,worldZ,Material.FOUNDATION));
                if (isBuildingBoundary(worldX,worldZ,masks)) {
                    for (int y=1; y<=INFERRED_WALL_HEIGHT; y++) {
                        boolean window=(y==2||y==3||y==6||y==7)&&windowCell(worldX,worldZ);
                        placements.add(new Placement(worldX,base+y,worldZ,window?Material.WINDOW:Material.WALL));
                    }
                }
                placements.add(new Placement(worldX,base+INFERRED_WALL_HEIGHT+1,worldZ,Material.ROOF));
    ...
    private static int[] buildingBases(ChunkFeatureRasterizer.ChunkMask mask,int[] surfaces) {
        ...
        for (int start=0;start<256;start++) {
            int sx=start%16, sz=start/16;
            if (visited[start]||mask.surfaceAt(sx,sz)!=ChunkFeatureRasterizer.Surface.BUILDING) continue;
            var queue=new ArrayDeque<Integer>(); var cells=new ArrayList<Integer>();
            queue.add(start); visited[start]=true; int base=surfaces[start];
            while(!queue.isEmpty()) {
                int cell=queue.removeFirst(), x=cell%16, z=cell/16; cells.add(cell); base=Math.max(base,surfaces[cell]);
                int[] neighbours={cell-1,cell+1,cell-16,cell+16};
                for(int next:neighbours) {
                    if(next<0||next>=256||visited[next]) continue;
                    int nx=next%16,nz=next/16;
    ...
    private static boolean adjacentToRoad(int x,int z,Map<Long,ChunkFeatureRasterizer.ChunkMask> masks) {
        return featureAt(x-1,z,masks)==ChunkFeatureRasterizer.Surface.ROAD||featureAt(x+1,z,masks)==ChunkFeatureRasterizer.Surface.ROAD
                ||featureAt(x,z-1,masks)==ChunkFeatureRasterizer.Surface.ROAD||featureAt(x,z+1,masks)==ChunkFeatureRasterizer.Surface.ROAD;
    }
    private static boolean isBuildingBoundary(int x,int z,Map<Long,ChunkFeatureRasterizer.ChunkMask> masks) {
        return featureAt(x-1,z,masks)!=ChunkFeatureRasterizer.Surface.BUILDING||featureAt(x+1,z,masks)!=ChunkFeatureRasterizer.Surface.BUILDING
                ||featureAt(x,z-1,masks)!=ChunkFeatureRasterizer.Surface.BUILDING||featureAt(x,z+1,masks)!=ChunkFeatureRasterizer.Surface.BUILDING;
    }
    private static ChunkFeatureRasterizer.Surface featureAt(int worldX,int worldZ,Map<Long,ChunkFeatureRasterizer.ChunkMask> masks) {
        var mask=masks.get(key(Math.floorDiv(worldX,16),Math.floorDiv(worldZ,16)));
        return mask==null?ChunkFeatureRasterizer.Surface.NONE:mask.surfaceAt(Math.floorMod(worldX,16),Math.floorMod(worldZ,16));
    }
